// Generated by CoffeeScript 1.6.3
(function() {
  var BOOLEAN_ATTRIBUTES, appendTo, bind, bindModel, debug, log, preprocess, render, tinywings, travel,
    __hasProp = {}.hasOwnProperty;

  debug = false;

  log = function() {
    if (debug) {
      return console.log.apply(console, arguments);
    }
  };

  travel = function(node, callback) {
    var done, next, stop;
    node = node.firstChild;
    stop = false;
    done = function() {
      return stop = true;
    };
    while (node) {
      log("travel " + node);
      next = node.nextSibling;
      callback(node, done);
      if (!stop) {
        travel(node, callback);
      }
      node = next;
    }
  };

  preprocess = function(node) {};

  bind = function(tw, attr, up) {
    var attrLink, firstAttr;
    firstAttr = attr.split('.')[0];
    tw.updaters[firstAttr] = tw.updaters[firstAttr] || [];
    tw[firstAttr] = tw[firstAttr] || function(val, o, parent) {
      var updater, _i, _len, _ref;
      _ref = tw.updaters[firstAttr];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        updater = _ref[_i];
        updater(val, o, parent);
      }
    };
    if (attr.indexOf('.') > -1) {
      attrLink = attr.split('.');
      firstAttr = attrLink.shift();
      tw.updaters[firstAttr].push(function(val, o, parent) {
        var atr, _i, _len;
        for (_i = 0, _len = attrLink.length; _i < _len; _i++) {
          atr = attrLink[_i];
          val = val[atr];
        }
        up(val, o, parent);
      });
    } else {
      tw.updaters[firstAttr].push(up);
    }
  };

  appendTo = function(el) {
    var child, next;
    child = this.frag.firstChild;
    while (child) {
      next = child.nextSibling;
      el.appendChild(child);
      child = next;
    }
    return this;
  };

  bindModel = function(model, attrs) {
    var attr, _i, _len,
      _this = this;
    this.render(model.toJSON());
    for (_i = 0, _len = attrs.length; _i < _len; _i++) {
      attr = attrs[_i];
      model.on("change:" + attr, function() {
        var m;
        m = model.toJSON();
        return _this[attr](m[attr], m);
      });
    }
    return this;
  };

  render = function(model) {
    var key, value;
    for (key in this) {
      if (!__hasProp.call(this, key)) continue;
      value = this[key];
      if (['render', 'frag', 'appendTo', 'bindModel', 'updaters'].indexOf(key) === -1) {
        this[key](model[key], model);
      }
    }
    return this;
  };

  BOOLEAN_ATTRIBUTES = ['disabled', 'readonly'];

  tinywings = function(tpl) {
    var frag, tw;
    log('START BIND');
    tw = {};
    frag = document.createElement('div');
    frag.innerHTML = tpl;
    tw.frag = frag;
    tw.appendTo = appendTo;
    tw.bindModel = bindModel;
    tw.render = render;
    tw.updaters = {};
    travel(frag, function(node, done) {
      var attr, binder, child, first, innerTpl, key, last, match, matches, newData, newNode, next, o, parent, type, value, _fn, _i, _j, _len, _len1, _ref, _ref1;
      if ((_ref = node.dataset) != null ? _ref.bind : void 0) {
        binder = node.dataset.bind;
        binder = binder.split(':');
        type = binder.shift();
        attr = binder.join(':');
        switch (type) {
          case 'text':
            log("text-bind to " + node + " with " + attr);
            bind(tw, attr, function(val) {
              node.innerHTML = val + '';
              return log("text-refrash to " + node + " with " + val);
            });
            break;
          case 'attr':
            log("attr-bind to " + node + " with " + attr);
            attr = JSON.parse(attr);
            for (key in attr) {
              value = attr[key];
              if (BOOLEAN_ATTRIBUTES.indexOf(key) > -1) {
                (function(key, value) {
                  return bind(tw, value, function(val) {
                    if (BOOLEAN_ATTRIBUTES.indexOf(key) > -1) {
                      node[key] = !!val;
                    } else {
                      node[key] = val;
                    }
                    return log("attr-refrash to " + node + " with " + val);
                  });
                })(key, value);
              }
            }
            break;
          case 'value':
            log("value-bind to " + node + " with " + attr);
            bind(tw, attr, function(val) {
              node.value = val;
              return log("value-refrash to " + node + " with " + val);
            });
            break;
          case 'if':
            done();
            innerTpl = node.innerHTML;
            log("if-bind to " + node + " with " + attr);
            bind(tw, attr, function(val, o) {
              var child, innerDomTpl, next;
              node.innerHTML = '';
              log("if-refrash to " + node + " with " + val);
              if (!val) {
                return;
              }
              innerDomTpl = tinywings(innerTpl);
              child = innerDomTpl.frag.firstChild;
              while (child) {
                next = child.nextSibling;
                node.appendChild(child);
                child = next;
              }
              for (key in innerDomTpl) {
                if (!__hasProp.call(innerDomTpl, key)) continue;
                value = innerDomTpl[key];
                if (key !== 'frag') {
                  if (o[key] != null) {
                    innerDomTpl[key](o[key], o, node);
                  }
                }
              }
            });
            break;
          case 'ifnot':
            done();
            innerTpl = node.innerHTML;
            log("if-bind to " + node + " with " + attr);
            bind(tw, attr, function(val, o) {
              var child, innerDomTpl, next;
              node.innerHTML = '';
              log("if-refrash to " + node + " with " + val);
              if (!!val) {
                return;
              }
              innerDomTpl = tinywings(innerTpl);
              child = innerDomTpl.frag.firstChild;
              while (child) {
                next = child.nextSibling;
                node.appendChild(child);
                child = next;
              }
              for (key in innerDomTpl) {
                if (!__hasProp.call(innerDomTpl, key)) continue;
                value = innerDomTpl[key];
                if (key !== 'frag') {
                  if (o[key] != null) {
                    innerDomTpl[key](o[key], o, node);
                  }
                }
              }
            });
            break;
          case 'with':
            done();
            innerTpl = node.innerHTML;
            log("with-bind to " + node + " with " + attr);
            bind(tw, attr, function(val) {
              var child, innerDomTpl, next;
              node.innerHTML = '';
              log("foreach-refrash to " + node + " with " + val);
              innerDomTpl = tinywings(innerTpl);
              child = innerDomTpl.frag.firstChild;
              while (child) {
                next = child.nextSibling;
                node.appendChild(child);
                child = next;
              }
              for (key in innerDomTpl) {
                if (!__hasProp.call(innerDomTpl, key)) continue;
                value = innerDomTpl[key];
                if (['render', 'frag', 'appendTo', 'bindModel', 'updaters'].indexOf(key) === -1) {
                  if (val[key] != null) {
                    innerDomTpl[key](val[key], val, node);
                  }
                }
              }
            });
            break;
          case 'foreach':
            done();
            innerTpl = node.innerHTML;
            log("foreach-bind to " + node + " with " + attr);
            bind(tw, attr, function(val) {
              var child, innerDomTpl, item, next, _i, _len;
              node.innerHTML = '';
              log("foreach-refrash to " + node + " with " + val);
              for (_i = 0, _len = val.length; _i < _len; _i++) {
                item = val[_i];
                innerDomTpl = tinywings(innerTpl);
                child = innerDomTpl.frag.firstChild;
                while (child) {
                  next = child.nextSibling;
                  node.appendChild(child);
                  child = next;
                }
                for (key in innerDomTpl) {
                  if (!__hasProp.call(innerDomTpl, key)) continue;
                  value = innerDomTpl[key];
                  if (['render', 'frag', 'appendTo', 'bindModel', 'updaters'].indexOf(key) === -1) {
                    if (item[key]) {
                      innerDomTpl[key](item[key], item, node);
                    }
                  }
                }
              }
            });
        }
      } else if (node.nodeType === 3 && /{{[^}]*}}/.test(node.data)) {
        log("text-bind to " + node);
        matches = node.data.match(/{{[^}]*}}/g);
        o = {};
        for (_i = 0, _len = matches.length; _i < _len; _i++) {
          match = matches[_i];
          o[match] = match;
        }
        matches = Object.keys(o);
        newData = node.data;
        parent = node.parentNode;
        _fn = function(attr, parent) {
          return bind(tw, attr, function(val, o, p) {
            var nodes, _k, _len2;
            p || (p = parent);
            nodes = p.childNodes;
            for (_k = 0, _len2 = nodes.length; _k < _len2; _k++) {
              node = nodes[_k];
              if ((node.data != null) && node.data.indexOf("data-bind='_text:" + attr + "'") > -1) {
                node.nextSibling.data = val;
                log("text-refrash to " + node + " with " + val);
              }
            }
          });
        };
        for (_j = 0, _len1 = matches.length; _j < _len1; _j++) {
          match = matches[_j];
          _ref1 = /{{([^}]*)}}/.exec(match), binder = _ref1[0], attr = _ref1[1];
          newData = newData.replace(new RegExp(binder, 'g'), "<!-- data-bind='_text:" + attr + "' -->" + binder + "<!-- -->");
          _fn(attr, parent);
        }
        newNode = document.createElement('div');
        newNode.innerHTML = newData;
        first = child = newNode.firstChild;
        while (child) {
          next = child.nextSibling;
          if (child === first) {
            parent.replaceChild(child, node);
          } else {
            if (parent.lastChild === last) {
              parent.appendChild(child);
            } else {
              parent.insertBefore(child, last.nextSibling);
            }
          }
          last = child;
          child = next;
        }
      }
    });
    log('END BIND');
    return tw;
  };

  if (typeof Backbone !== "undefined" && Backbone !== null) {
    Backbone.tinywings = tinywings;
  } else {
    this.tinywings = tinywings;
  }

}).call(this);
